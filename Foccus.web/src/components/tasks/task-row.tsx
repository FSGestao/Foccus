...
// status com o mesmo ícone/cor do select do painel de detalhe, seta pra
// expandir o checklist inline e realce quando selecionada (seleção múltipla).
export function TaskRow({
  ...
  onDelete: () => void;
  onToggleSubtask: (itemId: string) => void;
}) {
  ...
  const hasSubtasks = (task.checklist?.length ?? 0) > 0;
    ...
    >
      ...
      >
        <button
          ...
          // Antes de concluída, um verde já bem visível (era var(--pb-text-dim),
          // cinza neutro) — deixa claro que esse botão é o de concluir. Mais
          // claro que o verde vivo (#10b981) de quando já concluída, pra não
          // confundir os dois estados.
          style={{ background: "transparent", border: "none", color: done ? "#10b981" : "#34d399" }}
        >
          ✓
        </button>
        <button
        ...
        {hasSubtasks ? (
          <button
            ...
            className="flex h-[18px] w-[18px] shrink-0 items-center justify-center p-0"
            style={{ background: "transparent", border: "none", color: "var(--pb-accent)" }}
          >
            <svg width={11} height={11} viewBox="0 0 16 16" fill="currentColor">
              {isExpanded ? <path d="M3 5l5 7 5-7z" /> : <path d="M5 3l7 5-7 5z" />}
            </svg>
          </button>
        ) : (
      ...
      {isExpanded && hasSubtasks && (
        <div
          className="flex flex-col gap-1.5 py-2.5 pl-16 pr-3"
          style={{ background: "var(--pb-glass-strong-card)" }}
        >
          {task.checklist!.map((item) => (
            <div
              ...
              title="Alternar status da subtarefa"
            >
              <input type="checkbox" readOnly checked={item.done} className="pointer-events-none h-[13px] w-[13px]" />
              <span
                className="text-[12.5px]"
                style={
                  item.done
                    ? { color: "var(--pb-text-dim)", textDecoration: "line-through" }
                    : { color: "var(--pb-text)" }
                }
              >
                {item.text}
